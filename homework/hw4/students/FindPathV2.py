...
def astar(problem, maze):
    ...
    heapq.heappush(frontier, (heuristic(node.state, problem.getGoalState()) + node.pathCost, node))
    ...
    while frontier:
        ...
        if node.state not in explored:
            ...
            for (cost, state, direction) in successors[node.state]:
                childNode = Node(cost, state, direction, node)
                # add to the frontier: in the end
                heapq.heappush(frontier,(heuristic(childNode.state, problem.getGoalState()) + childNode.pathCost, childNode))
    ...
    while frontier:
        ...
        if node.state not in explored:
            ...
            for (cost, state, direction) in successors[node.state]:
                childNode = Node(cost, state, direction, node)
                # add to the frontier
                heapq.heappush(frontier, (heuristic(childNode.state, problem.getGoalState()), childNode))
...
def dfs(problem, maze):
    ...
    while frontier:
        ...
        if node.state not in explored:
            ...
            for (cost, state, direction) in successors[node.state]:
                childNode = Node(cost, state, direction, node)
                # add to the frontier: in the front
